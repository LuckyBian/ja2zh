...
def run_gpt_training(args):
    ...
    config["train"]["half_weights_save_dir"] = "model"

    config["train_semantic_path"] = s1_dir + "/6-name2semantic.tsv"
    config["train_phoneme_path"] = s1_dir + "/2-name2text-0.txt"
    config["output_dir"] = f"{s1_dir}/logs_s1"

    # Write the modified configuration to a temporary file
    tmp_config_path = "train/gpt/tmp/tmp_s1.yaml"
    ...
    # 运行核心代码
    cmd = f'{args.python_exec} train/gpt/s1_train.py --config_file "{tmp_config_path}"'
    print(cmd)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu_numbers", type=str, default="7")
